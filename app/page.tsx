...
/** Build exportable single-file HTML for any list of tabs (inline CSS/JS, no classes) */
...
(function(){
  ...
  var n=${/* JS uses the rendered tab count */ ""}${useTabs.length}, idx=[${useTabs.map((_, i) => i).join(",")}];
...
export default function HomePage() {
  ...
  // Choose what to export: all tabs or just one tab
  const [mode, setMode] = useState<ExportMode>({ type: "all" });

  // The generated/exportable HTML
  const [out, setOut] = useState("");

  // One-time placeholder so typing in the editor doesn't reload the preview
  useEffect(() => {
    setOut(`<!doctype html><html lang="en"><meta charset="utf-8"><title>Tabs</title></html>`);
  }, []);

  // Keep selected single-tab index in bounds if tabs are removed
  useEffect(() => {
    if (mode.type === "one" && (mode.index < 0 || mode.index >= tabs.length)) {
      setMode({ type: "all" });
    }
  }, [tabs.length, mode]);

  // Compute HTML to export on demand
  const computed = useMemo(() => {
    if (mode.type === "all") return buildHtml(tabs, label);
    const t = tabs[mode.index];
    // Slightly adjust label so single-tab export is obvious
    const singleLabel = `${label} — ${t?.title ?? "Tab"}`;
    return buildHtml([t], singleLabel);
  }, [mode, tabs, label]);
            ...
            {tabs.map((t, i) => (
              <fieldset key={i} className="border rounded p-3">
                ...
                <div className="flex gap-2 mb-2">
                  <button type="button" className="px-2 py-1 border rounded" onClick={() => move(i, -1)}>
                    ↑
                  </button>
                  <button type="button" className="px-2 py-1 border rounded" onClick={() => move(i, 1)}>
                    ↓
        ...
        <div className="flex flex-col gap-3 lg:h-[78vh] min-h-[60vh]">
          ...
          <div className="border rounded p-2 sticky top-0 bg-[var(--background)] z-10">
            <div className="flex flex-wrap gap-2 items-center">
              <button className="px-3 py-2 border rounded" type="button" onClick={generate}>
                Generate
              </button>
              <button
                className="px-3 py-2 border rounded"
              ...
              <button className="px-3 py-2 border rounded" type="button" onClick={download}>
                Download
              </button>
